...
**증상**: MDRM에서 시스템을 인식하지 못합니다.

**원인**: NodeID 매핑이 잘못되었을 수 있습니다.

**해결방법**:

1. 설정 파일에서 NodeID를 재확인합니다.
2. 백업센터에서 생성한 NodeID와 비교합니다.
3. 올바른 NodeID로 수정 후 Agent를 재시작합니다.

### Agent가 두 곳에서 동시 실행

**증상**: MDRM에서 시스템 상태가 불안정합니다.

**원인**: 백업센터와 운영센터에서 동일한 NodeID의 Agent가 동시 실행되고 있을 수 있습니다.

**해결방법**:

1. 백업센터 Agent를 종료합니다.
2. 운영센터 Agent만 실행 유지합니다.
3. 복제 프로세스를 재검토합니다.
...
!!! warning "반드시 확인해야 할 사항"
    ...
    ✅ **NodeID 매핑**
    - 백업센터에서 생성한 NodeID를 정확히 기록해야 합니다.
    - 운영센터 설정 파일에 정확히 입력해야 합니다.
    
    ✅ **Agent 중복 실행 방지**
    - 백업센터와 운영센터에서 동시 실행하면 안 됩니다.
    - 복제 전 반드시 한쪽 Agent를 종료해야 합니다.
    
    ✅ **MDRM 서버 정보**
    - 운영센터 MDRM 서버 IP를 정확히 설정해야 합니다.
    - 네트워크 연결 가능 여부를 확인해야 합니다.
    
    ✅ **복제 프로세스 문서화**
    - 복제 절차를 명확히 문서화해야 합니다.
    - 담당자 간 공유가 필수입니다.
...
- OS 이미지 복제 환경에서 사용할 수 있습니다.
- 백업센터와 운영센터를 분리하여 관리할 수 있습니다.
...
- 설정 과정이 매우 복잡합니다.
- NodeID 매핑 오류 시 심각한 문제가 발생할 수 있습니다.

❌ **관리 부담**

- 수동으로 관리해야 합니다.
- 복제 프로세스 추가 작업이 필요합니다.
