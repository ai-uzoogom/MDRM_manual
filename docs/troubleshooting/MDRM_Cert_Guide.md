# 🔒 공인 인증서 사용 가이드 (MDRM v4.6.2.4+)

!!! info "가이드 대상"
    본 가이드는 고객사의 공인 인증서(SSL)를 MDRM에 적용하기 위한 절차입니다. MDRM v4.6.2.4 이상 버전부터 지원하며, NGINX와 Tomcat 두 곳에 설정을 적용해야 합니다.
...
브라우저를 열고 MDRM 웹 콘솔에 접속(`https://<MDRM_IP>`)하여, 주소창의 자물쇠 아이콘을 클릭합니다.
