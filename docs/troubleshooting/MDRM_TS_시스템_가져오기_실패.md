...
MDRM 서버는 [가져오기] 기능을 수행할 때 Agent 통신 전에 대상 서버에 대한 기본적인 네트워크 연결을 확인하기 위해 **Ping (ICMP) 테스트**를 먼저 수행합니다. 
사내 방화벽이나 대상 서버의 인바운드 규칙에서 Ping을 차단하고 있다면, 실제 통신(Agent 포트 등)이 가능하더라도 **"사전 점검 실패"**로 간주하여 등록 절차를 중단하게 됩니다.
