...
!!! info "학습 안내"
    MDRM 솔루션 운영 중 보안 정책이나 포트 충돌로 인해 포트 번호를 변경해야 하는 경우의 조치 방법입니다. **MDRM 서버 자체의 접속 포트**를 변경하는 방법과, **Agent 통신 포트 변경 시 MDRM 서버에서 수행해야 할 작업**을 상세히 안내합니다.
...
## **2. Agent 통신 포트 변경 (MDRM 서버 작업)**

Agent의 기본 포트(20080)를 변경하여 사용할 경우, MDRM 서버에서 수행해야 하는 작업 가이드입니다.

!!! info "통신 포트 개요"
    Agent의 기본 포트 정보는 **20080**을 사용하고 있습니다. 고객사에서 20080 포트 사용 불가 시 Agent의 포트 번호를 변경하여 사용하는 방법을 공유합니다. 
...
**적용 범위**: 시스템 Agent 연결 테스트, 시스템 요약, 점검 작업, 워크플로우 (4.6.1 이하)
...
!!! success "최종 확인"
    모든 포트 변경 작업이 완료된 후에는 반드시 웹 콘솔의 **[시스템 관리]** 메뉴에서 **'Agent 연결 테스트'**를 수행하여 통신 상태가 정상(OK)인지 확인하시기 바랍니다.
